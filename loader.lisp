    ...
    `(let ((,l ,line))
       (or ,@(loop for (command . body) in commands
                   ...
                   collect (case regex
                             ...
                             (T `(cl-ppcre:register-groups-bind ,args (,regex ,l)
                                   ,@body
                                   T))))))))
  ...
  (let ((vertices (mkarray '(or null vec)))
    ...
    (labels ((add (element target)
             ...
             (map-refs (refspecs function)
               (loop for refspec in (cl-ppcre:split " +" refspecs)
            ...
            do (with-processing-case line
                 ...
                 (("p( -?[0-9]+)+" refspecs)
                  (let ((point (make-instance 'point)))
                    (map-refs refspecs (lambda (vertex) (add vertex (vertices point))))
                    (add point (geometry (mesh)))))
                 (("l( -?[0-9]+(/(-?[0-9]+)?)?)+" refspecs NIL NIL)
                  (let ((line (make-instance 'line)))
                    (map-refs refspecs (lambda (vertex &optional texture)
                                         (add vertex (vertices line))
                                         (add texture (textures line))))
                    (add line (geometry (mesh)))))
                 (("f( -?[0-9]+(/(-?[0-9]+)?(/(-?[0-9]+)?)?)?)+" refspecs NIL NIL NIL NIL)
                  (let ((face (make-instance 'face)))
                    (map-refs refspecs (lambda (vertex &optional texture normal)
